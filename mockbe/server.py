...
def validate_file_access(file_meta: dict, user: dict, password_header: str):
    ...
    if status == "expired":
         return jsonify({"error": "File expired", "expiredAt": file_meta.get("availableTo"), "message": "File has expired"}), 410
    ...
    if status == "pending" and not is_owner:
         ...
         return jsonify({"error": "File not yet available", "availableFrom": file_meta.get("availableFrom"), "hoursUntilAvailable": hours_until, "message": "File not yet available"}), 423
    ...
    if file_meta["passwordProtected"]:
        if not password_header:
             return jsonify({"error": "Password required", "message": "This file is password-protected. Please provide the password parameter"}), 403
        if password_header != file_meta.get("password"):
             return jsonify({"error": "Incorrect password", "message": "The file password is incorrect"}), 403
...
def register():
    ...
    if not username or not email or not password:
        return jsonify({"error": "Validation error", "message": "username, email and password are required"}), 400
...
def login_totp():
    ...
    if not cid or not code:
        return jsonify({"error": "Validation error", "message": "cid and code are required"}), 400
    
    email = totp_temp_sessions.get(cid)
    if not email:
        return jsonify({"error": "Unauthorized", "message": "Login session expired. Please restart the login flow."}), 401

    if code != MOCK_TOTP_CODE:
        return jsonify({"error": "Unauthorized", "message": "Invalid or expired TOTP code"}), 401
...
def totp_verify():
    ...
    if not code:
        return jsonify({"error": "Validation error", "message": "code is required"}), 400

    if code != MOCK_TOTP_CODE:
        return jsonify({"error": "Invalid TOTP code", "message": "The provided code is incorrect or expired"}), 400
...
def get_available_files():
    ...
    active_public_files = []
    for file_meta in files.values():
        if file_meta.get("isPublic") and get_file_status(file_meta) == "active":
...
def admin_cleanup():
    ...
    if not user or user.get("role") != "admin":
        return jsonify({"error": "Forbidden", "message": "You don't have permission to perform cleanup"}), 403
...
def upload_file():
    ...
    if not upload_file:
        return jsonify({"error": "Validation error", "message": "File is required"}), 400
    ...
    try:
        ...
        if available_from >= available_to:
            return jsonify(
                {"error": "Validation error", "message": "availableFrom must be before availableTo and within allowed policy window"}
            ), 400
    except Exception:
        return jsonify({"error": "Validation error", "message": "Invalid datetime format, use ISO format"}), 400
    ...
    if not is_public and not user:
         return jsonify({"error": "Unauthorized", "message": "Private uploads (isPublic=false/sharedWith) require authentication"}), 401
