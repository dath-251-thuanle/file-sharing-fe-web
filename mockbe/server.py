...
def upload_file():
    ...
    if size > max_bytes:
        return (
            jsonify({"error": "File too large", "maxFileSizeMB": policy.get("maxFileSizeMB")}),
            413,
        )

    # Parse form fields
    is_public = str(request.form.get("isPublic", "false")).lower() in ("1", "true", "yes", "on")
    ...
    try:
        ...
        if available_from and available_to and available_from >= available_to:
            return jsonify({"error": "availableFrom must be earlier than availableTo"}), 400
    except Exception:
        return jsonify({"error": "Invalid datetime format, use ISO format"}), 400

    shared_with = request.form.getlist("sharedWith") or []
    enable_totp = str(request.form.get("enableTOTP", "false")).lower() in ("1", "true", "yes", "on")

    # build file metadata
    file_id = str(uuid.uuid4())
    owner_email = user.get("email") if user else None
    share_link = f"http://localhost:3000/s/{file_id}"
    ...
    response = {"success": True, "file": file_meta, "message": "Upload successful (mock)"}
    if totp_setup:
